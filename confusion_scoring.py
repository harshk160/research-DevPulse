...
Running this script (from `src/`, same as the other pipeline scripts):
  ...
  - writes confusion_scores_<repo>.txt (the complete ranked table) per repo
  Move the generated query_log_*.json and confusion_scores_*.txt files into
  data/query_logs/ and data/confusion_scores/ respectively, same as you do
  for the other scripts' output.
"""
...
def save_full_report(repo_name: str, scored_funcs: list[dict], stats: dict, output_dir: str = ".") -> str:
    """Writes the complete (non-truncated) ranked table for one repo to its own .txt file
    in the current working directory (same convention as the other pipeline scripts) —
    move it into data/confusion_scores/ afterward."""
