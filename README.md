...
`manual_evaluation.py` (blind human scoring, Task 11) is also included as part of
the pipeline code, but its output isn't committed here — human rubric scores are
reviewer-specific and not reproducible data, so they're left out of `data/`.
...
└── data/
    ...
    ├── query_logs/                           # Task 9 output — per-repo retrieval hit counts
    │   ├── query_log_httpx.json
    ...
    └── evaluation/
        ...
        └── devpulse_llm_judge_scores.csv     # Task 13 output — LLM-judge rubric scores
```
...
# 6. Manually score the batch output (blind A/B, console-based) — optional,
#    output is reviewer-specific and not committed to data/
...
logs, the full 72-generation batch output
([devpulse_evaluation_batch.json](https://github.com/harshk160/research-DevPulse/blob/main/data/evaluation/devpulse_evaluation_batch.json)),
the LLM-judge rubric scores
([devpulse_llm_judge_scores.csv](https://github.com/harshk160/research-DevPulse/blob/main/data/evaluation/devpulse_llm_judge_scores.csv)),
and the complete (non-truncated) confusion-score ranking for every function
in each repo — the paper's tables show only the top 10 per repo. Human rubric
scores from `manual_evaluation.py` are not included, since they're
reviewer-specific and not reproducible. The underlying source code for the
three evaluated repositories is not redistributed here — it's pulled live
from GitHub by `ingest.py`.
